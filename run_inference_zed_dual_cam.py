...
def open_svo(path: Path) -> sl.Camera:
    ...
    p.coordinate_system = sl.COORDINATE_SYSTEM.IMAGE  # matches ArUco calib convention
...
class DualSvoStream:
    ...
    def __init__(self, svo1: Path, svo2: Path, calib_path: Path,
        ...
        # ArUco extrinsics are stored in RIGHT_HANDED_Y_UP convention
        # (generated by aruco_calibration.py with C @ T_opencv @ C),
        # with translations in metres (sl.UNIT.METER).
        #
        # UmeTrack opens ZED SDK with COORDINATE_SYSTEM.IMAGE (Y-down)
        # and its tracker expects camera_to_world_xf translations in mm
        # (see tracker.py _make_inputs: *= MM_TO_M to feed the regressor).
        #
        # Two conversions needed:
        #   1. Convention: Y_UP → IMAGE  via  C @ T @ C
        #   2. Units:      metres → mm   via  translation *= 1000
        _C = np.diag([1., -1., -1., 1.])

        def _yup_m_to_image_mm(T_yup_m: np.ndarray) -> np.ndarray:
            T_img = _C @ T_yup_m @ _C       # Y_UP → IMAGE convention
            T_img[:3, 3] *= 1000.0           # metres → mm
            return T_img

        self._T_cam1_world = _yup_m_to_image_mm(self._extrinsics[sn1])
        self._T_cam2_world = _yup_m_to_image_mm(self._extrinsics[sn2])

        # Per-camera stereo intrinsics and right-in-left transforms.
        # ZED SDK returns stereo_transform in IMAGE convention (matching
        # our open_svo coordinate_system) with translations in metres.
        # Convert translation to mm to match the extrinsics above.
        ...
        T_cam1_right_in_left[:3, 3] *= 1000.0   # metres → mm
        T_cam2_right_in_left[:3, 3] *= 1000.0   # metres → mm

        # World-space poses for all 4 cameras (IMAGE convention, mm)
